...
export const register = createAsyncThunk("auth/register", async(user, thunkAPI) => {
    try{
        return await authService.register //state.user = action.payload below
    }catch(e){
        const message = (e.res && e.res.data && e.res.data.message) || e.message || e.toString()
...
export const authSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(register.fufilled, (state, action) => {
                state.isLoading = false
